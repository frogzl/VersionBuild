...
#include "../Common.h"
SourceCodeCreate::SourceCodeCreate(ServiceData *pD) :m_pD(pD)
{
}
...
void SourceCodeCreate::process_task()
{
	...
	dbSourceCodeData.repertoryname = m_pD->request_data().jData["repertoryname"].asString();
	string sTmp = m_pD->request_data().jData["branchname"].asString();
	...
	sTmp = m_pD->request_data().jData["name"].asString();
	...
	sTmp = m_pD->request_data().jData["description"].asString();
	...
	dbSourceCodeData.startversion = m_pD->request_data().jData["versionstart"].asInt();
	dbSourceCodeData.currentversion = dbSourceCodeData.startversion;
	dbSourceCodeData.status = m_pD->request_data().jData["status"].asInt();
	dbSourceCodeData.buildstate = m_pD->request_data().jData["buildstate"].asInt();
	dbSourceCodeData.buildstatemask = m_pD->request_data().jData["buildstatemask"].asInt();
	if (!dbSourceCode.create(dbSourceCodeData)->exec())
	{
		m_pD->set_respond_back(HTTP_INTERNAL, "1", "create sourcecode failed", "");
		return;
	}
	jsonRespondData["guid"] = dbSourceCodeData.guid;

	Json::Value jsonBuildRules = m_pD->request_data().jData["buildrules"];
	for (int nIndexBuildRule = 0; nIndexBuildRule < jsonBuildRules.size(); nIndexBuildRule++)
	{
		...
		{
			m_pD->set_respond_back(HTTP_INTERNAL, "1", "create buildrule_deploy failed", "");
			return;
		}
		...
		{
			m_pD->set_respond_back(HTTP_INTERNAL, "1", "create buildrule failed", "");
			return;
		}
		...
		for (int nIndexDepend = 0; nIndexDepend < jsonDepends.size(); nIndexDepend++)
		{
			...
			{
				m_pD->set_respond_back(HTTP_INTERNAL, "1", "create buildrule_depend failed", "");
				return;
			}
	...
	m_pD->set_respond_back(HTTP_OK, "0", "create sourcecode resource success!", "", jsonRespondData);
}
