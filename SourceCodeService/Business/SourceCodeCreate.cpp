...
void SourceCodeCreate::process_task()
{
	...
	dbSourceCodeData.repertoryname = m_rdInData.jData["repertoryname"].asString();
	string sTmp = m_rdInData.jData["branchname"].asString();
	g2u(sTmp.c_str(), sTmp.length(), m_szBuf, Buffer_Size);
	dbSourceCodeData.branchname = m_szBuf;
	sTmp = m_rdInData.jData["name"].asString();
	g2u(sTmp.c_str(), sTmp.length(), m_szBuf, Buffer_Size);
	dbSourceCodeData.name = m_szBuf;
	sTmp = m_rdInData.jData["description"].asString();
	g2u(sTmp.c_str(), sTmp.length(), m_szBuf, Buffer_Size);
	dbSourceCodeData.discription = m_szBuf;
	dbSourceCodeData.startversion = m_rdInData.jData["versionstart"].asInt();
	dbSourceCodeData.currentversion = dbSourceCodeData.startversion;
	dbSourceCodeData.status = m_rdInData.jData["status"].asInt();
	dbSourceCodeData.buildstate = m_rdInData.jData["buildstate"].asInt();
	...
	Json::Value jsonBuildRules = m_rdInData.jData["buildrules"];
