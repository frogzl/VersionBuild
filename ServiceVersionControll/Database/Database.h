...
#define Define_Table_Create_Function_Filter_BigNumber(Name) {char szTmp[255];std::stringstream stream;stream << mm(pdData.)##mm(Name);stream.get(szTmp, 255);sQuery += (string(szTmp) + ",");}
...
#define Define_Table_Update_Function_Filter_Number(Name) 	if (vecfilter.count(dd(Name)) != 0){\
		...
		bIsFirst = false;\
		char szTmp[255];\
		itoa(mm(pdData.)##mm(Name), szTmp, 10);\
		sQuery += (mm("`")##dd(Name)##mm("`=") + string(szTmp) + " ");}
#define Define_Table_Update_Function_Filter_BigNumber(Name) 	if (vecfilter.count(dd(Name)) != 0){\
		if (!bIsFirst) sQuery += ",";\
		bIsFirst = false;\
		char szTmp[255];\
		std::stringstream stream; stream << mm(pdData.)##mm(Name); stream.get(szTmp, 255);\
		sQuery += (mm("`")##dd(Name)##mm("`=") + string(szTmp) + " ");}
